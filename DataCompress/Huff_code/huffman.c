...
calculate_huffman_codes(SymbolFrequencies * pSF)
{
	...
	/* 
		快速排序,得到排序后的子叶数组,接下来在进行合并排序的递归过程.后面是用循环完成的.
	*/
	...
	for(i = 0; i < n - 1; ++i)
	{
		...
		/*
			循环做快速排序,每做一次排序,新建一个节点,将最小概率的两个节点合并链接在该节点上.
		*/
...
//step3:add by me for huffman statistics
/*
统计各符号出现的概率
*/
int huffST_getSymFrequencies(SymbolFrequencies *SF, huffman_stat *st,int total_count)//统计各符号出现的概率的函数
{
	int i,count =0;
	for(i = 0; i < MAX_SYMBOLS; ++i)
	{	
...
统计各符号对应的码长和码字
*/
int huffST_getcodeword(SymbolEncoder *se, huffman_stat *st)//统计各符号对应的码长和码字 的函数
{
	unsigned long i,j;
...
输出所有的统计信息
*/
void output_huffman_statistics(huffman_stat *st,FILE *out_Table)//输出所有的统计信息
{
	unsigned long i,j;
	unsigned char c;
	...
	{	
		...
		fprintf(out_Table,"%f\t",st->freq[i]);		//概率
...
huffman_encode_file(FILE *in, FILE *out,FILE *out_Table)
{
	...
	int rc;
	unsigned int symbol_count;
	//step:add by me for huffman statistics
	huffman_stat hs;
	//end by me
	...
	//step3:add by me for huffman statistics，...get the frequency of each symbol
	...
	//step3:add by me for huffman statistics... output the statistics to file
